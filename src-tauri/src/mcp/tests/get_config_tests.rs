...
use super::fixtures::{
    test_env, ANTIGRAVITY_CONFIG_JSON, CLAUDE_CODE_CONFIG_JSON, COPILOT_CLI_CONFIG_JSON,
    OPENAI_CODEX_CONFIG_TOML, VSCODE_COPILOT_CONFIG_JSON,
};
use super::EnvGuard;
...
#[rstest]
fn get_agent_mcp_config_command_reads_antigravity(
    test_env: (TempDir, EnvGuard, std::sync::MutexGuard<'static, ()>),
) {
    let (temp_dir, _env_guard, _lock) = test_env;
    ...
    fs::create_dir_all(config_path.parent().unwrap()).unwrap();
    fs::write(&config_path, ANTIGRAVITY_CONFIG_JSON).unwrap();

    let config = get_agent_mcp_config_command("antigravity".to_string()).unwrap();
    assert_eq!(config.servers.len(), 2);
