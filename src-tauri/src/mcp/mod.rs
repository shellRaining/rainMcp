...
mod tests {
    use super::{
        get_agent_mcp_config_command, update_agent_mcp_config_command, BaseMcpConfig,
        LocalMcpConfig, McpConfig, McpServerConfig, RemoteMcpConfig,
    };
    use std::collections::HashMap;
    use std::env;
...
"#,
    ...
    fn update_agent_mcp_config_command_errors_on_invalid_toml() {
        ...
        let config = McpConfig { servers };
        let err =
            update_agent_mcp_config_command("openai-codex".to_string(), config).unwrap_err();
